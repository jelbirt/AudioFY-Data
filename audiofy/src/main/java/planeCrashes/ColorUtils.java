 ...
 * @author Xiaoxiao Li
 * 
 * Modified 1/22/23 @author Benjamin Elbirt
 * 	Added distance_wb method to figure out relative distance of a color to white and to black.
 * 
 */
public class ColorUtils {
	
	public double[] distance_wb(Color color) {
		Color w = Color.white;
		Color b = Color.black;
		double[] distance = new double[] { 0.0, 0.0 };
		double dW = 0.0;
		double dB = 0.0;
		
		dW = Math.sqrt((
				((color.getRed() - w.getRed()) * (color.getRed() - w.getRed())) +
				((color.getBlue() - w.getBlue()) * (color.getBlue() - w.getBlue())) +
				((color.getGreen() - w.getGreen()) * (color.getGreen() - w.getGreen()))
		));
		dB = Math.sqrt((
				((color.getRed() - b.getRed()) * (color.getRed() - b.getRed())) +
				((color.getBlue() - b.getBlue()) * (color.getBlue() - b.getBlue())) +
				((color.getGreen() - b.getGreen()) * (color.getGreen() - b.getGreen()))
		));
		
		distance[0] = dW; distance[1] = dB;
		return distance;
	}
